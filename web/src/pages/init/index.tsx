...
export default function InitProjectPage() {
    ...
    const steps: { key: Step; title: string; icon: React.ReactNode }[] = [
        { key: 'project-info', title: '项目信息', icon: <Projector className="h-5 w-5" /> },
        { key: 'database', title: '数据库配置', icon: <DatabaseIcon className="h-5 w-5" /> },
        { key: 'confirmation', title: '确认', icon: <CheckCircle className="h-5 w-5" /> },
    ];
    ...
    const testDatabaseConnection = async () => {
        ...
        // 实际应用中这里会有真实的连接测试逻辑
        try {
            await form.validateFields(['dbType', 'dbHost', 'dbPort', 'dbName', 'dbUser']);
    ...
    // 提交表单
    const handleSubmit: FormProps<FormData>['onFinish'] = async (values) => {
        setIsSubmitting(true);

        try {
            // 模拟提交数据
            console.log('项目配置:', values);
                                ...
                                <>
                                    ...
                                    <div className="mt-6">
                                        <Button
                                            ...
                                            icon={connectionTest.status === 'testing' ?
                ...
                <header className="bg-white border-b border-gray-200 sticky top-0 z-10">
                    <div className="container mx-auto px-4 py-3 flex justify-between items-center">
                        <div className="flex items-center space-x-2">
                            ...
                            <h1 className="text-xl font-bold text-gray-800">项目初始化助手</h1>
                        </div>
